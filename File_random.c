...
// Отправляет полученное число в функцию  stabilisation а затем записывет его в source файл и в source массив
...
void rand_writing_target_array(int* source_array, int size_source_array, FILE *target_file) {                                                                                         
    ...
    while (counter != size_source_array) {
        ...
        if (*(source_array + i) != 0) {                               
            fprintf(target_file, "%d ", *(source_array + i));        // Находим рандомное i до тех пор пока не наткнемся на не "пустую" ячейку
